...
def predict_from_survival_model(data, survival_model, cat_features, survival_threshold):
    ...
    missing_cols = train_cols_surv_model[
        ~train_cols_surv_model.isin(data_to_surv_model.columns)
    ]
    ...
    days_to_churn = surv_prediction.loc[
        (surv_prediction >= survival_threshold).values
    ].index[-1]
