...
def plot_confusion_matrix(_pipe, _X, _y, ax, dataset_name="Test"):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                xticklabels=_pipe["model"].classes_, yticklabels=_pipe["model"].classes_, ax=ax)
...
def get_top_cluster_features(data, _pipe, top_n_features):  
    ...
    for cluster in range(k):
        ...
        cluster_features[f'{cluster}'] = pd.DataFrame({'Features': feature_names, 'Importance': feature_importance})
