...
def make_feature_engineering(df):
    df.loc[:, ["commission_xs", "product_x", "product_y"]] = df.loc[
        :, ["commission_xs", "product_x", "product_y"]
    ].fillna(0)

    df.loc[:, "is_xs"] = np.where((df["product_x"] + df["product_y"]) > 0, 1, 0)
    ...
    df.loc[:, "LTV"] = (df.loc[:, "months_as_client"] * df["commission_base"]) + df[
        "commission_xs"
    ].fillna(0)
    return df
